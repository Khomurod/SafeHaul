...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          '**Status: Approved layout, one open element choice.** The composition below is',
          'built entirely from approved primitives.',
          '',
          'The reason this used to say "no approved link primitive" is gone: `Link`,',
          '`ButtonLink` and `IconButtonLink` are approved and exported from',
          '`@design-system/components`, and `ButtonLink` is an anchor wearing the `Button`',
          'shape — exactly the control this pattern wanted. What is still open is narrower',
          'and is about the example below: its back affordance is a `Button` with an',
          '`onClick`, and a back affordance is semantically a *link* — it should be openable',
          'in a new tab and announced as a link. Use `ButtonLink` with an `href` in product',
          'code; this story has not been re-recorded yet.',
          '',
          'Either way, do not invent a local link component, and do not use an `<a>` with an',
          '`onClick` and no `href`.',
          '',
          '### Anatomy',
          '',
