...
function BatchTimer({ startTime }) {
    ...
    useEffect(() => {
        const calculate = () => {
            ...
            // Get Target Time: Today at 7 AM CT
            let target = new Date();
            target.toLocaleString('en-US', { timeZone: 'America/Chicago' }); // Ensure reference

            // Use a helper to get CT time precisely
            ...
            // If it's after 7 AM CT today, the next rotation is tomorrow at 7 AM CT
            let nextRotation = new Date(now);
            if (ctHour >= 7) {
                nextRotation.setDate(nextRotation.getDate() + 1);
            }

            // Set next rotation to exactly 7:00:00 AM in CT
            // We can't easily set 'CT' on a JS Date, but we can target the 7 AM mark
            // by calculating the difference.

            // Simpler approach: Calculate until a fixed hour in the local time 
            // translated from CT.

            const ctDate = new Date(now.toLocaleString('en-US', { timeZone: 'America/Chicago' }));
            const nextCtRotation = new Date(ctDate);
            nextCtRotation.setHours(7, 0, 0, 0);

            if (ctDate.getHours() >= 7) {
                nextCtRotation.setDate(nextCtRotation.getDate() + 1);
            }

            const diff = nextCtRotation.getTime() - ctDate.getTime();
