/**
 * The seventeen System Health diagnostic steps: the ordered list the runner
 * walks, and what each step actually does against the live project —
 * infrastructure probes, simulated applicant flows, security checks, and the
 * cleanup that purges everything the run created. Extracted verbatim from
 * `useSystemHealth.js`; the hook owns the runner, the pause/resume state and
 * the persisted progress, and each step reaches its world through the one
 * context argument (the accumulated test data, the updater, and the logger).
 */
...
export async function executeHealthStep(stepId, { currentData, updateData, addLog, getData }) {
        switch (stepId) {
            case 'init': {
            ...
            case 'cleanup': {
                ...
                // A live read, as the original did with `testDataRef.current`:
                // cleanup must see everything every earlier step recorded.
                const data = getData();
