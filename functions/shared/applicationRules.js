// functions/shared/applicationRules.js
//
// Company-configurable Application Rules: what happens when an answer, a missing
// answer or a date needs more than "is it filled in".
//
// WHY THIS EXISTS
// ---------------
// Every carrier runs the same wizard, but they do not want the same application.
// One refuses expired licences outright, another only wants to be told; one
// does not hire recent graduates, another does; one must have three years of
// employment history before a recruiter sees the file, another accepts gaps and
// asks later. Before this module those differences could only be met by
// changing the code for everybody — which is how "Wenze needs X" becomes "every
// carrier gets X".
//
// The rules live on `companies/{id}.applicationRules`, are projected to the
// public profile the apply page reads, and are evaluated by ONE function in
// three places: each wizard step (to tell the applicant, immediately), the
// browser's final pre-flight (to route them back to the right page) and
// `submitGuestApplication` (to refuse what a resumed draft or a hand-built
// payload tries to slip past). The server reads the company's rules from its
// own record, never from the client.
//
// EVERY DEFAULT REPRODUCES THE PRE-RULES BEHAVIOUR. A company that never opens
// the settings sees exactly the application it had. Stricter behaviour exists
// only where a company switched it on.
//
// MIRRORED, DELIBERATELY. `src/config/applicationRules.js` is the browser copy:
// the body between the `--- body ---` and `--- exports ---` markers is
// byte-identical, only the module syntax differs, and
// `applicationRules.parity.test.js` fails if the two bodies drift by a single
// character. The shared vectors in `applicationRules.vectors.json` drive both
// suites as well.
//
// This module must stay free of `firebase-admin`: the frontend test job loads it
// with no functions dependencies installed.

const STANDARD_SECTIONS = require('./applicationSections.json');
const CATALOG = require('./applicationRulesCatalog.json');
const { resolveGate } = require('./applicationDefinition');
const { computeEmploymentCoverage } = require('./employmentCoverage');
const { dateStatus, isOngoingToken, parseApplicationDate, toIsoDay } = require('./applicationDates');

// --- body ---------------------------------------------------------------------
// Identical to src/config/applicationRules.js. Edit both, or the parity test fails.
...
module.exports = {
    APPLICATION_RULES_CATALOG: CATALOG,
    ENFORCEMENT_LEVELS,
    EXPERIENCE_VALUES,
