...
| `npm run test:visual` | **yes**, since 2026-08-25 | A change to how anything *looks*, across 95 catalog subjects and 15 real screens at both widths. **Known gap:** the e-doc field editor and the sandbox wizard are not among the 15, so changes confined to them are guarded by `check:visual-contract` measurements rather than by a photograph |
