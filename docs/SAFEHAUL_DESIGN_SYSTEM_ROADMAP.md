...
| **Filled icon tile** | **Open**, found 2026-09-05 | Found while building the card disclosure's `leading` slot. A square, filled, ~40px tile holding one inverse glyph appears at four sites — `EmailSettingsTab`, `DashboardHeader`, `ReviewChangePortal`, `CompanySidebar` — all written in `--ds-*` roles, so no rule sees them. `StatusMedallion` is the near miss and does not fit: it is a CIRCLE and its tones are the six status roles, where every one of these is a rounded square filled with the primary action colour. P-5's disc audit did not cover it, having deliberately scoped itself to round shapes. Not built here: `leading` takes whatever the feature passes and hides it, and putting a tile primitive *inside* `Disclosure` would be the wrong home for a shape three of its four sites do not use |
