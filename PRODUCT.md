...
Voice, as established in the current copy and worth preserving: plain, specific,
...
selling you something." That candour is an asset, not a liability.

## Evidence on Hand

- Four real customers shown as a logo strip: STL Truckers, True Nation, I&S
  Transportation, TopHire Recruiting Agency (`landing/assets/images/company_logos/`).
  They may be shown as carriers hiring on SafeHaul. They may **not** be presented
  as endorsing it.
- Seven real product screenshots captured from a fixture tenant ("Ridgeline
  Carriers"), in `landing/assets/images/screenshots/`: pipeline, dashboard,
  campaigns, edocs, signing consent, super-admin, driver-application-mobile.
  Regenerated by `npm run capture:landing-screenshots` against
  `VITE_E2E_TEST_MODE=1` — never against production, which once leaked real
  driver names onto a public page.
- A live, daily-publishing News & Insights blog, fetched at runtime.
